...
#if 0
  //===--------------------------------------------------------------------===//
  // Raj:: AxC definitions to hold error bounds
  //===--------------------------------------------------------------------===//
  class AxCErrorBounds;
  class AxCErrorBounds {
	  typedef SmallVector<int, 8>  ArgErrorMapTy;
	  ArgErrorMapTy ArgErrorMap; 
  public:
	  AxCErrorBounds() {}
	  void setErrorBound(int ArgNo, int ErrorPercentage) {
		  ArgErrorMap[ArgNo] = ErrorPercentage;
	  }
	  int getErrorBound(int ArgNo) {
		  return ArgErrorMap[ArgNo];
	  }
...
#if 0
  ///Raj:: AxC related Metadata
  AxCErrorBounds *ErrorBounds;
  void AddAxCMetaData() {
	  if (ErrorBounds) {  
	  }
  }
