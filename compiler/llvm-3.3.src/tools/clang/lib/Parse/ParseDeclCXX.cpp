...
void Parser::ParseCXXMemberSpecification(SourceLocation RecordLoc,
  ...
  if (TagDecl) {
    ...
    while (Tok.isNot(tok::r_brace) && Tok.isNot(tok::eof)) {
	  ...
	  // Raj:: AxC pragma call!
	  if (Tok.is(tok::annot_pragma_axc_precision)) {
		  //fprintf(stderr, "Parsing axc_precision in ParseCXXMemberSpecification\n");
		ParseAxCPrecisionDirective();
		continue;
	  }
#if 0
	  if (Tok.is(tok::annot_pragma_axc_memo)) {
		  fprintf(stderr, "Parsing axc_memo in ParseCXXMemberSpecification\n");
		ParseAxCMemoDirective();
		continue;
	  }
#endif
	  if (Tok.is(tok::annot_pragma_axc)) {
		  //fprintf(stderr, "Parsing axc in ParseCXXMemberSpecification\n");
		ParseAxCDirective();
		continue;
	  } 
