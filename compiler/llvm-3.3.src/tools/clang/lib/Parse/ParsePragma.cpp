...
#if 0
//Raj AxC Pragma
/// \brief Handle '#pragma axc ...' when AxC is disabled.
///
void
PragmaNoAxCHandler::HandlePragma(Preprocessor &PP,
                                    PragmaIntroducerKind Introducer,
                                    Token &FirstTok) {
	fprintf(stderr,"Handle AxC pragma without -faxc flag...\n");
#if 0
  if (PP.getDiagnostics().getDiagnosticLevel(diag::warn_pragma_axc_ignored,
                                             FirstTok.getLocation()) !=
      DiagnosticsEngine::Ignored) {
	fprintf(stderr,"Handle AxC pragma ignored...\n");
    PP.Diag(FirstTok, diag::warn_pragma_axc_ignored);
    PP.getDiagnostics().setDiagnosticMapping(diag::warn_pragma_axc_ignored,
                                             diag::MAP_IGNORE,
                                             SourceLocation());
  }
#endif
...
//Raj AxC Pragma
/// \brief Handle '#pragma axc ...' when AxC is enabled.
///
void
PragmaAxCHandler::HandlePragma(Preprocessor &PP,
                                  PragmaIntroducerKind Introducer,
                                  Token &FirstTok) {
  //fprintf(stderr, "Handle AxC pragma...\n");
  SmallVector<Token, 16> Pragma;
  Token Tok;
  Tok.startToken();
  Tok.setKind(tok::annot_pragma_axc);
  Tok.setLocation(FirstTok.getLocation());
  ...
  SourceLocation EodLoc = Tok.getLocation();
  Tok.startToken();
  Tok.setKind(tok::annot_pragma_axc_end);
  Tok.setLocation(EodLoc);
  Pragma.push_back(Tok);
...
//Raj AxC Pragma
/// \brief Handle '#pragma axc_memo ...' when AxC is enabled.
///
void
PragmaAxCMemoHandler::HandlePragma(Preprocessor &PP,
                                  PragmaIntroducerKind Introducer,
                                  Token &FirstTok) {
  //fprintf(stderr, "Handle AxC Memoize pragma....\n");
  SmallVector<Token, 16> Pragma;
  Token Tok;
  Tok.startToken();
  Tok.setKind(tok::annot_pragma_axc_memo);
  Tok.setLocation(FirstTok.getLocation());
  ...
  SourceLocation EodLoc = Tok.getLocation();
  Tok.startToken();
  Tok.setKind(tok::annot_pragma_axc_memo_end);
  Tok.setLocation(EodLoc);
  Pragma.push_back(Tok);
...
//  Raj AxC Pragma
/// \brief Handle '#pragma axc_precision ...' when AxC is enabled.
///
void
PragmaAxCPrecisionHandler::HandlePragma(Preprocessor &PP,
                                  PragmaIntroducerKind Introducer,
                                  Token &FirstTok) {
  //fprintf(stderr, "Handle AxC Precision pragma...\n");
  SmallVector<Token, 16> Pragma;
  Token Tok;
  Tok.startToken();
  Tok.setKind(tok::annot_pragma_axc_precision);
  Tok.setLocation(FirstTok.getLocation());
  ...
  SourceLocation EodLoc = Tok.getLocation();
  Tok.startToken();
  Tok.setKind(tok::annot_pragma_axc_precision_end);
  Tok.setLocation(EodLoc);
  Pragma.push_back(Tok);
