...
Parser::Parser(Preprocessor &pp, Sema &actions, bool skipFunctionBodies)
  ...
  //Raj:: AxC pragma handler  
  AxCPrecisionHandler.reset(new PragmaAxCPrecisionHandler());
  PP.AddPragmaHandler(AxCPrecisionHandler.get());
  AxCMemoHandler.reset(new PragmaAxCMemoHandler());
  PP.AddPragmaHandler(AxCMemoHandler.get());
  AxCHandler.reset(new PragmaAxCHandler());
  PP.AddPragmaHandler(AxCHandler.get());
...
Parser::~Parser() {
  ...
  // Raj:: AxC Pragma
  PP.RemovePragmaHandler(AxCPrecisionHandler.get());
  AxCPrecisionHandler.reset();

  PP.RemovePragmaHandler(AxCHandler.get());
  AxCHandler.reset();

  PP.RemovePragmaHandler(AxCMemoHandler.get());
  AxCMemoHandler.reset();
...
Parser::ParseExternalDeclaration(ParsedAttributesWithRange &attrs,
  ...
  //Raj:: axc pragmas
  case tok::annot_pragma_axc_precision:
	  //fprintf(stderr, "Parsing axc_precision in ParseExternalDeclaration\n");
    ParseAxCPrecisionDirective();
    return DeclGroupPtrTy();

#if 0 // axc_memo only goes with statements
  case tok::annot_pragma_axc_memo:
	  fprintf(stderr, "Parsing axc_memo in ParseExternalDeclaration\n");
    ParseAxCMemoDirective();
    return DeclGroupPtrTy();
#endif 
  case tok::annot_pragma_axc:
	  //fprintf(stderr, "Parsing axc in ParseExternalDeclaration\n");
    ParseAxCDirective();
    return DeclGroupPtrTy();
