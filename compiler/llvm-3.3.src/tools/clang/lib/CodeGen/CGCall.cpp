...
RValue CodeGenFunction::EmitCall(const CGFunctionInfo &CallInfo,
  ...
  // Raj AxC Memoize code generation along with the instruction
  if(TargetDecl->AxCMemoize) {
	//fprintf(stderr, "Generating the call instruction...\n");
	// dump the annotation here -- boom
	AxCInfo *AxCContext = (AxCInfo *)(TargetDecl->AxCMemoize);
	llvm::Instruction *insn = CS.getInstruction();
	llvm::LLVMContext  &C = getLLVMContext();
	SmallVector<llvm::Value *, 16> InErrorPairs;
	for(std::vector<InEntry>::iterator it = AxCContext->In.begin(), ite = AxCContext->In.end(); it != ite; it++) {
		llvm::Value *InErrorPair[2];
		InErrorPair[0] = llvm::ConstantInt::get(llvm::Type::getInt32Ty(C), it->ArgNum);
		InErrorPair[1] = llvm::ConstantFP::get(llvm::Type::getFloatTy(C), it->errorBound);
		llvm::MDNode *InErrorPairMD = llvm::MDNode::get(C, InErrorPair);
		InErrorPairs.push_back(InErrorPairMD);
	}
	llvm::MDNode *InErrorPairsMD = llvm::MDNode::get(C, InErrorPairs);
	insn->setMetadata(llvm::LLVMContext::MD_axc_memoize_in, InErrorPairsMD);

	SmallVector<llvm::Value *, 8> Outs;
	for(std::vector<OutEntry>::iterator it = AxCContext->Out.begin(), ite = AxCContext->Out.end(); it != ite; it++) {
		llvm::Value *out = llvm::ConstantInt::get(llvm::Type::getInt32Ty(C), it->ArgNum);
		llvm::MDNode *outMD = llvm::MDNode::get(C, out);
		Outs.push_back(outMD);
	}
	llvm::MDNode *outsMD = llvm::MDNode::get(C, Outs);
	insn->setMetadata(llvm::LLVMContext::MD_axc_memoize_out, outsMD);
	((Decl *)TargetDecl)->AxCMemoize = NULL;
	//fprintf(stderr, "Added meta data in call instruction...\n");
  }
