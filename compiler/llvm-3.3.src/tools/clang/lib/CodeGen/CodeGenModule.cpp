...
void CodeGenModule::SetLLVMFunctionAttributesForDefinition(const Decl *D,
  ...
  // Raj: add the AxC  attribute
  if(D->hasAttr<AxCAttr>())  {
	  //fprintf(stderr, "Setting axc attribute for function\n");
	  //F->addFnAttr(llvm::Attribute::get(F->getContext(), llvm::Attribute::AxC, 0));
	  F->addFnAttr(llvm::Attribute::AxC);
  }

  // Raj: add the AxCPrecision = "precision_reduce"  attribute
  if(D->hasAttr<AxCPrecisionAttr>())  {
	  //fprintf(stderr, "Setting axc_precision attribute for function: \n");
	  F->addFnAttr(llvm::Attribute::AxCPrecision);
  }
