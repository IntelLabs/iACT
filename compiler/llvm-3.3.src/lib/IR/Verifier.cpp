...
void Verifier::VerifyAttributeTypes(AttributeSet Attrs, unsigned Idx,
  ...
  for (AttributeSet::iterator I = Attrs.begin(Slot), E = Attrs.end(Slot);
...
#if 0
	 /* Temporary fix:: Added by Raj for AxC and AxCPrecision  handling */
	if (I->getKindAsEnum() == Attribute::AxC ||
        I->getKindAsEnum() == Attribute::AxCPrecision) {
			if (!isFunction)
    ...
    if (I->getKindAsEnum() == Attribute::NoReturn ||
		...
		I->getKindAsEnum() == Attribute::AxC || /* Raj: AxC attribute to be passed */
        I->getKindAsEnum() == Attribute::AxCPrecision) {
      if (!isFunction)
          CheckFailed("Attribute '" + I->getKindAsString() +
                      "' only applies to functions!", V);
...
void Verifier::VerifyCallSite(CallSite CS) {
  ...
  // Raj:: verify the axc_memoize_in meta data
  if (MDNode *AxCINMemos = I->getMetadata(LLVMContext::MD_axc_memoize_in)) {
	unsigned NumOperands = AxCINMemos->getNumOperands();
	Assert1((NumOperands > 0), "Empty MD_axc_memoize_in arguments!", AxCINMemos);
	for (unsigned j = 0; j <NumOperands; j++) {
		if (MDNode *AxCINMemo = dyn_cast<MDNode>(AxCINMemos->getOperand(j))) {
			unsigned NumINOperands = AxCINMemo->getNumOperands();
			Assert1((NumINOperands % 2 == 0), "Unfinished axc in errorBounds!", AxCINMemo);

			unsigned NumINAxCErrorBounds = NumINOperands / 2;
			Assert1((NumINAxCErrorBounds >= 1), "It should have at least one axc in errorbound!", AxCINMemo);

			for (unsigned i = 0; i < NumINAxCErrorBounds; ++i) {
				ConstantInt *argNum = dyn_cast<ConstantInt>(AxCINMemo->getOperand(2*i));
				Assert1(argNum, "The lower limit must be a positive integer!", argNum);
				APInt argNumV = argNum->getValue();

				ConstantFP *errorBound = dyn_cast<ConstantFP>(AxCINMemo->getOperand(2*i + 1));
				Assert1(errorBound, "The upper limit must be a positve floating point value between (0...100)!", errorBound);
				APFloat errorBoundV = errorBound->getValueAPF();
				/* TODO: Add more checks:
					argNum should be less than the number of arguments 
					errorBound should be between 0 and 100 */
			}
		}
	}
  }
  // Raj:: verify the axc_memoize_out meta data
  if (MDNode *AxCOUTMemos = I->getMetadata(LLVMContext::MD_axc_memoize_out)) {
	unsigned NumOperands = AxCOUTMemos->getNumOperands();
	Assert1((NumOperands > 0), "Empty MD_axc_memoize_out arguments!", AxCOUTMemos);
	for (unsigned j = 0; j < NumOperands; j++) {
		if (MDNode *AxCOUTMemo = dyn_cast<MDNode>(AxCOUTMemos->getOperand(j))) {
			unsigned NumOUTAxCOperands = AxCOUTMemo->getNumOperands();
			Assert1((NumOUTAxCOperands >= 1), "It should have at least one axc out errorbound!", AxCOUTMemo);
...
void Verifier::visitInstruction(Instruction &I) {
  ...
  //Raj: MD_axc_memoize should only be for call sites 
  MDNode *AxCMDIN = I.getMetadata(LLVMContext::MD_axc_memoize_in);
  Assert1(!AxCMDIN || isa<CallInst>(I) || isa<InvokeInst>(I), "axc_memoize are only for call sites!", &I);
  MDNode *AxCMDOUT = I.getMetadata(LLVMContext::MD_axc_memoize_out);
  Assert1(!AxCMDOUT || isa<CallInst>(I) || isa<InvokeInst>(I), "axc_memoize are only for call sites!", &I);
